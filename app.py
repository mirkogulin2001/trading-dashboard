...
with st.sidebar:
    ...
    # LÍMITE DE SEGURIDAD: Máximo 20,000 para estabilidad total
    n_simulaciones = st.slider("Simulaciones", 1000, 20000, 5000)
    ...
    if st.button("🚀 CORRER SIMULACIÓN", type="primary"):
        with st.spinner(f'Calculando {n_simulaciones:,.0f} escenarios futuros...'):
            try:
                ...
                # 1. GRÁFICO PROYECCIÓN EQUITY (Matplotlib por eficiencia)
        ...
        with st.spinner('Procesando historial...'):
            try:
                ...
                if len(pnl_real) == 0:
                    ...
                    # 4. GRÁFICOS REALES (LAYOUT DIVIDIDO 3:1)
                    plt.style.use('dark_background')
                    col_chart_main, col_chart_dist = st.columns([3, 1])
                    
                    # --- COLUMNA 1: Equity + Drawdown ---
                    with col_chart_main:
                    ...
                    # --- COLUMNA 2: Histograma PnL + Normal ---
                    with col_chart_dist:
                        ...
                        ax_hist.axvline(mu, color='yellow', linestyle='--', label='Media')
