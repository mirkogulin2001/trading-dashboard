...
with st.sidebar:
    ...
    # CORRECCIÓN DE ESTABILIDAD: Máximo 10,000 para evitar crash de memoria
    n_simulaciones = st.slider("Simulaciones", 500, 10000, 2000)
    ...
    if st.button("🚀 CORRER SIMULACIÓN", type="primary"):
        with st.spinner(f'Calculando {n_simulaciones:,.0f} escenarios futuros...'):
            try:
                ...
                # Solo ploteamos 200 para no saturar visualmente
